...
{
	if (!parentChit) return;	// should only happen in test bed. and it will get names later anyway, if there is some edge case.
...
void ItemComponent::NewsDestroy( const GameItem* item )
{
	...
	if ( item->keyValues.Get( ISC::destroyMsg, &msg ) == 0 ) {
		history->Add( NewsEvent( msg, pos, parentChit->GetItemID(), destroyer ? destroyer->GetItemID() : 0));
	}
}
